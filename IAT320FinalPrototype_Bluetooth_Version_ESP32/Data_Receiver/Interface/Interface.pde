...
int valZScaled;
//filter Data
static final int numReadings = 5;
float totalValue = 0;
...
void patient(){
  ...
  if(myPort.available() >0){
    valZScaled = myPort.read();
    valueZ = ((float)valZScaled/10.0 - 11.0);
  }
  
  //filter data
  totalValue = totalValue - readings[readIndex];
  readings[readIndex] = valueZ;
  totalValue = totalValue + readings[readIndex];
  ...
  angle = map(averageValue, -4, 10, PI/8, -PI/2 +PI/10);
  ...
  if(mouseX > 80 && mouseX < 80 + 60 && mouseY > 50 && mouseY < 50 + 30){
   ...
   if(mousePressed){
    ...
    //count Rep Number
    if(averageValue > thresholdHigh && isCount == false && holdTime >= repTime){
      repNumber = repNumber+1;
      isCount = true;
    }else if(averageValue < thresholdLow){
      isCount = false;
    }   
